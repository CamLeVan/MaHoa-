...
public class Thread_10 extends Thread{
    ...
    public Thread_10(allOclockController allController) {
        this.allOclock = allController;
    }
    public Thread_10(oclockControllers controller) {
    ...
    private void updateAllTime() {
        ...
        Platform.runLater(() -> {
            allOclock.getGMT_10().setText(timeH);
        });
    }
    private void updateTime() {
        String timeH = getTimeH();
        Platform.runLater(() -> {
            controller.getMui_10().setText(timeH);

        });
    }
    public String getTimeH(){
        ZonedDateTime time = ZonedDateTime.now(ZoneId.of("GMT+10"));
