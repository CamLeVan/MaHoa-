...
public class Thread_4 extends Thread{
    ...
    public Thread_4(allOclockController allController) {
        this.allOclock = allController;
    }
    public Thread_4(oclockControllers controller) {
    ...
    private void updateAllTime() {
        ...
        Platform.runLater(() -> {
            allOclock.getGMT_4().setText(timeH);
        });
    }
    private void updateTime() {
        String timeH = getTimeH();
        Platform.runLater(() -> {
            controller.getMui_4().setText(timeH);

        });
    }
    public String getTimeH(){
        ZonedDateTime time = ZonedDateTime.now(ZoneId.of("GMT+4"));
