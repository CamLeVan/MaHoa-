...
public class Thread_3 extends Thread{
    ...
    public Thread_3(allOclockController allController) {
        this.allOclock = allController;
    }
    public Thread_3(oclockControllers controller) {
    ...
    private void updateAllTime() {
        ...
        Platform.runLater(() -> {
            allOclock.getGMT_3().setText(timeH);
        });
    }
    private void updateTime() {
        String timeH = getTimeH();
        Platform.runLater(() -> {
            controller.getMui_3().setText(timeH);

        });
    }
    public String getTimeH(){
        ZonedDateTime time = ZonedDateTime.now(ZoneId.of("GMT+3"));
