...
public class Thread_9 extends Thread{
    ...
    public Thread_9(allOclockController allController) {
        this.allOclock = allController;
    }
    public Thread_9(oclockControllers controller) {
    ...
    private void updateAllTime() {
        ...
        Platform.runLater(() -> {
            allOclock.getGMT_9().setText(timeH);
        });
    }
    private void updateTime() {
        String timeH = getTimeH();
        Platform.runLater(() -> {
            controller.getMui_9().setText(timeH);

        });
    }
    public String getTimeH(){
        ZonedDateTime time = ZonedDateTime.now(ZoneId.of("GMT+9"));
